...
def get_list(file: str) -> list[dict[str, str]]:
    ...
    """
    names_list = ['Наименование товара', 'Количество товара', 'Стоимость товара за 1 шт.']
    list_from_txt = []
    goods_dicts = []
    with open(file, encoding='utf-8') as txt_file:
        for line in txt_file:
            list_from_txt.append(line.strip().split('\t'))
        for goods in list_from_txt:
            goods_dicts.append(dict(zip(names_list, goods)))
        return goods_dicts
...
def get_total_price(file: str) -> int:
    ...
    for goods in goods_list:
        try:
            price_of_goods = int(goods['Количество товара']) * int(goods['Стоимость товара за 1 шт.'])
        except Exception:
            price_of_goods = 0
