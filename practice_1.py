...
def get_list_from_json(file: str) -> list[dict[str, str | int]]:
    """
    ...
    with open(file, 'r', encoding='utf-8') as json_file:
        list_from_json: list[dict[str, str | int]] = json.load(json_file)
        return list_from_json


def get_numbers_of_students(students: list[dict[str, str | int]]) -> int:
    """
...
def get_max_age_student(students: list[dict[str, str | int]]) -> tuple[str, int, str]:
    """
    ...
    older_student: list[dict[str, str | int]] = [student for student in students if student.get('возраст') == max_age]
    for max_age_student in older_student:
        return max_age_student.get('имя'), max_age_student.get('возраст'), max_age_student.get('город')


def get_students_by_discipline(students: list[dict[str, str | int]]) -> dict[str, int]:
    """
...
def file_analysis(file: str) -> None:
    ...
    :return: None
    """
    list_of_students: list[dict[str, str | int]] = get_list_from_json(file)
...
def get_goods_list(csv_file: str) -> list[dict[str, str | int]]:
    """
    ...
    """
    goods_list: list[dict[str, str | int]] = []
...
def get_total_sales_amount(sales: list[dict[str, str | int]]) -> int:
    """
...
def get_most_popular_product(sales: list[dict[str, str | int]]) -> tuple[str, int]:
    """
...
def sales_by_month(sales: list[dict[str, str | datetime]]) -> dict[str, int]:
    """
...
def file_analysis(file: str) -> None:
    ...
    :return: None
    """
    goods_list: list[dict[str, str | int]] = get_goods_list(file)
...
def get_employees_list(json_file: str) -> list[dict[str, str | int]]:
    """
...
def get_performance_list(csv_file: str) -> list[dict[str, str | int]]:
    """
    ...
    """
    performance_lists: list[dict[str, str | int]] = []
    with open(csv_file, 'r', encoding='utf-8') as file:
        reader = csv.reader(file)
        reader = list(map(lambda y: list(map(lambda x: x.strip(), y)), reader))
        i = 1
...
def get_employees_performance(employees_list: list[dict[str, str | int]],
                              performance_list: list[dict[str, str | int]]) -> list[dict[str, str | int]]:
    """
...
def get_average_performance(employees: list[dict[str, str | int]]) -> float:
    """
...
def get_max_performance(employees: list[dict[str, str | int]]) -> tuple[str, int]:
    """
    ...
    the_best_employee: list[dict[str, str | int]] = [employee for employee in employees if
...
def performance_analysis(json_file: str, csv_file: str) -> None:
    ...
    :return: None
    """
    employees: list[dict[str, str | int]] = get_employees_list(json_file)
    performance: list[dict[str, str | int]] = get_performance_list(csv_file)
    employees_performance: list[dict[str, str | int]] = get_employees_performance(employees, performance)
